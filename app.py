...
st.markdown("""
<div class="footer">
    <p>Built with ❤️ using Firecrawl and Streamlit</p>
...
# st.markdown("""# Scrape any Websites using Prompts""", unsafe_allow_html=True)

# st.markdown(
#         """
                
#         This app allows you to convert ANY website into an API using Firecrawl.
...
#     st.caption("Define fields to extract from the website")
    
#     # Allow removal of fields
#     if len(st.session_state.schema_fields) > 1 and st.button("Remove Last Field ➖"):
#         st.session_state.schema_fields.pop()
#         st.rerun()
...
#     if len(st.session_state.schema_fields) < 10:  # Increased limit to 10 fields
...
#                             'prompt': prompt,
#                             'schema': schema,
#                             'max_results': 20  # Ensure we try to get multiple results
#                         }
#                         data = app.extract(
#                             [website_url],
#                             extract_params
#                         )
#                     else:
#                         # Use our enhanced project extraction
#                         data = extract_projects(app, website_url, prompt)
                    
#                     # if schema:
#                     #     extract_params['schema'] = schema
                    
#                     # # Debug information
#                     # st.info(f"Extracting data from: {website_url}")
...
#                     formatted_result = format_extraction_result(data)
#                     placeholder = stream_text(formatted_result)
#                     st.session_state.messages.append({"role": "assistant", "content": formatted_result})
